...
def analyze_business(data: BusinessRequest):
    ...
    # ==================================================
    # GOVERNMENT LOAN SCHEME ANALYSIS
    # ==================================================

    beneficiary_contribution = data.investment * 0.10
    eligible_loan = data.investment * 0.90
    ...
    elif data.investment <= 5000000:

        scheme_name = "Term Loan Scheme"
        eligible_loan = min(eligible_loan, 4500000)
        interest_rate = 7.0
        repayment_period = (
            "Up to 8 years including moratorium period"
        )
    ...
    if (
        ...
        if monthly_interest_rate > 0:
            ...
            emi = eligible_loan / loan_tenure_months

        monthly_emi = round(emi, 2)
        total_repayment = round(
            monthly_emi * loan_tenure_months,
            2
        )

        total_interest = round(
            total_repayment - eligible_loan,
            2
        )
    ...
    if data.investment <= 140000:
        ...
        matching_schemes.append({
            ...
            "maximum_loan": 125000,
            "eligible_loan": round(
                min(data.investment * 0.90, 125000),
                2
            ),
            "interest_rate": 6.5,
            "repayment_period": "3 years including 3-month moratorium",
        ...
        matching_schemes.append({
            ...
            "eligible_loan": round(
                min(data.investment * 0.90, 4500000),
                2
            ),
            "interest_rate": 7.0,
            "repayment_period": "Up to 8 years including moratorium period",
            "match_score": 95,
    ...
    return {
        ...
        "scheme_analysis": {
            ...
            "project_cost": round(data.investment, 2),
